...
def pdf_analysis():
    ...
    if request.method == 'POST':
        file = request.form # a multidict containing POST data
    
    try: 
        new_filename,text=textpdf_to_txt(file)
        #if text==    ---------- :
...
def ocr_pdf_to_text(filename):
    ...
    files=[
      ('file',(filename,open('https://www.bu.edu/geneva/files/2010/08/Easy_recipes.pdf')))
    ]
    headers = {}

    response = requests.request("POST", url, headers=headers, data=payload, files=files, auth=requests.auth.HTTPBasicAuth('8b819689-9f3b-11ed-a60f-ba1ca0caa142', ''))
    data=response.text
...
def text_to_speech(data):
    ...
    engine.save_to_file(data, new_filename)
    return new_filename
...
def makePDF(input_filename,summary,bottom_text,punctuation_filename,avg_word_length_filename,unique_word_types_filename,sentence_length_filename,reading_time):
    ...
    pdf.text(x,y_reading_time,'The average reading time for this document is: ' + reading_time)
...
def main(filename):
    try: 
        new_filename,text=textpdf_to_txt(filename)
        #if text==    ---------- :
         #   text=ocr_pdf_to_text(filename)
    except:
        new_filename,text=textpdf_to_txt(filename)
    ...
    #output_filename=makePDF(filename,summary,bottom_text,punctuation_filename,
                          #  avg_word_length_filename,unique_word_types_filename,
                          #  sentence_length_filename,time_to_read)
